...
void MyGLCanvas::drawScene() {
	...
	float blendFactor = textureBlend;
	bool useDiffuse = this->useDiffuse;
	...
	GLuint objectProgramID = myShaderManager->getShaderProgram("objectShaders")->programID;
    glUseProgram(objectProgramID);
	// glUseProgram(myShaderManager->getShaderProgram("objectShaders")->programID);
	glUniformMatrix4fv(glGetUniformLocation(objectProgramID, "modelMatrix"), 1, GL_FALSE, glm::value_ptr(modelMatrix));

	glUniformMatrix4fv(glGetUniformLocation(objectProgramID, "viewMatrix"), 1, GL_FALSE, glm::value_ptr(viewMatrix));
	glUniformMatrix4fv(glGetUniformLocation(objectProgramID, "perspectiveMatrix"), 1, GL_FALSE, glm::value_ptr(perspectiveMatrix));

	glm::mat3 normalMatrix = glm::transpose(glm::inverse(glm::mat3(modelMatrix)));
	glUniformMatrix3fv(glGetUniformLocation(objectProgramID, "normalMatrix"), 1, GL_FALSE, glm::value_ptr(normalMatrix));

	glUniform3fv(glGetUniformLocation(objectProgramID, "lightPosition"), 1, glm::value_ptr(eyePosition));

	glUniform1f(glGetUniformLocation(objectProgramID, "blendFactor"), blendFactor);

	glUniform1i(glGetUniformLocation(objectProgramID, "useDiffuse"), useDiffuse);
	...
	glUseProgram(environmentProgramID);

	glm::mat4 environmentModelMatrix = glm::scale(glm::mat4(1.0f), glm::vec3(7.0f));
	glUniformMatrix4fv(glGetUniformLocation(environmentProgramID, "modelMatrix"), 1, GL_FALSE, glm::value_ptr(environmentModelMatrix));
	glUniformMatrix4fv(glGetUniformLocation(environmentProgramID, "viewMatrix"), 1, GL_FALSE, glm::value_ptr(viewMatrix));
	glUniformMatrix4fv(glGetUniformLocation(environmentProgramID, "perspectiveMatrix"), 1, GL_FALSE, glm::value_ptr(perspectiveMatrix));
	glUniform1i(glGetUniformLocation(environmentProgramID, "environMap"), 0);
